const USERS = {
  ...
  jazeel:    { pass: 'leezaj',    name: 'Jazeel',    role: 'Finance · Strategic', photo: 'photos/Jazeel.png'   },
  hasnu:     { pass: 'unsah',     name: 'Hasnu',     role: 'Sales · Alpha Access', photo: 'photos/Hasnu.png'   },
};
...
function buildSparklines() {
  ...
  const DATA = [
    [35, 50, 42, 68, 58, 82, 100], [30, 45, 55, 50, 72, 78, 100],
    [55, 65, 62, 78, 72, 88, 100], [78, 88, 82, 94, 72, 100, 84],
  ...
  for (let i = 0; i < 5; i++) {
    ...
    DATA[i].forEach(v => {
      ...
      b.className = 'spk-bar'; b.style.height = v + '%';
      b.style.background = COLORS[i]; b.style.boxShadow = `0 0 4px ${COLORS[i]}99`;
      el.appendChild(b);
    });
  }
  ...
  if (rev) {
    [22, 35, 42, 38, 55, 64, 78, 100].forEach(v => {
      const b = document.createElement('div');
      Object.assign(b.style, { flex: '1', height: v + '%', borderRadius: '2px 2px 0 0', background: '#FFC400', opacity: '.85', boxShadow: '0 0 5px rgba(255,196,0,.5)' });
      rev.appendChild(b);
    });
  }
}
...
const JARVIS_CMDS = {
  ...
  'close': closeJarvis, 'dismiss': closeJarvis,
...
function startVoice() {
  ...
  recognition.onresult = e => {
    ...
    if (finalText) {
      ...
      let matched = false;
      for (const key of Object.keys(JARVIS_CMDS)) { if (cmd.includes(key)) { JARVIS_CMDS[key](); matched = true; break; } }
